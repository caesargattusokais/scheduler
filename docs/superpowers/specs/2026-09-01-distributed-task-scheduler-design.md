...
1. 核心调度循环(定时触发、触发/执行解耦、task/execution 两层、并发配额)
2. 可靠性三件套(超时、心跳/租约防孤儿、幂等重试 + 死信 DLQ)
3. 分片并行执行
4. 工作流依赖编排(DAG)

> 里程碑在同一代码库里演进,并非一次到位。每步都能用、能验证;后补极贵的可靠性在 M2 就位,不最后补。
...
- 看板选型(VictoriaMetrics + Grafana vs 原地起步)。
- scheduler 根目录名 / git 初始化时机。
