...
/** 当前登录者角色(来自 App 会话;ADMIN 才可管理操作者,其余仅可只读浏览)。 */
interface Props { meRole: 'OPERATOR' | 'ADMIN' | null }

export default function OperatorsPage({ meRole }: Props) {
      ...
      <div className="page-head">
        ...
        {!isAdmin && (
          <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">
            你正以 <span className="font-mono">{meRole}</span> 身份只读浏览,管理操作者需 ADMIN 账号
          </div>
        )}
      </div>
                ...
                <Fragment key={r.name}>
                  <tr>
                    ...
                    <td className="text-right">
                      <button className="btn btn-secondary" title="查看该操作者的活动会话"
                    ...
                    <tr>
                      <td colSpan={4} className="bg-slate-50">
                        <div className="flex items-center justify-between gap-4 px-4 py-3">
                          ...
                          {!sessLoading && isAdmin && sessions.length > 0 && (
